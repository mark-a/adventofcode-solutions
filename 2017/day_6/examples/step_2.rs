...
fn main() {
    ...
    let position = seen.iter().position(| s| s == &format!("{:?}", numbers)).unwrap();

    println!(" looping after {} steps, samle value as position {}: loop size {}", step_counter ,position, step_counter - position);
}
